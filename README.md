...
| 解析構築 | `create_analysis`, `assign_material`, `add_boundary_condition`, `add_load`, `add_constraint`, `create_mesh`, `validate_analysis` |
...
5. `add_boundary_condition`でfixedまたはdisplacementを、`add_load`でforce、pressureまたはgravityを追加します。targetsを省略した場合は現在のGUI選択を使います。`add_constraint`は既存クライアントの移行用です。
