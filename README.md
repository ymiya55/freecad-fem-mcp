...
4. `assign_material`でPa・kg/m³単位の材料値を設定します。1D beam／2D shellでは、`assign_element_geometry`で明示Edgeへのbeam断面・断面回転、または明示Faceへのshell板厚・offsetをSI単位で割り当てます。beam断面は矩形、円形、Pipe、楕円、Box、truss presetに対応します。shellの`formulation`は曲げ剛性を持つ`shell`または面内剛性だけの`membrane`です。
